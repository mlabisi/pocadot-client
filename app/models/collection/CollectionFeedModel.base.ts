 ...
 */
export const CollectionFeedModelBase = withTypedRefs<Refs>()(ModelBase
  .named('CollectionFeed')
  .props({
    __typename: types.optional(types.literal("CollectionFeed"), "CollectionFeed"),
    page: types.union(types.undefined, types.integer),
    collections: types.union(types.undefined, types.array(MSTGQLRef(types.late((): any => CollectionModel)))),
  })
  .views(self => ({
    get store() {
